  ...
  <div class="flex flex-col h-full w-full font-sans transition-colors duration-500 bg-[#F8FAFC] dark:bg-[#09090B] overflow-hidden">
    <div class="flex items-center gap-3 px-1 mb-2 shrink-0">
      ...
      <div class="flex items-baseline gap-2">
        <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Spectrum Analysis</h1>
    ...
    <div class="mb-3 bg-white dark:bg-[#111111] p-1.5 rounded-xl border border-slate-200 dark:border-zinc-800 flex flex-wrap gap-2 items-center justify-between shadow-sm shrink-0 transition-colors duration-300">
      <div class="flex flex-wrap items-center flex-1 gap-2 px-1 py-1 overflow-x-auto scrollbar-hide">
         <Select v-model="filterStore.selectedSite" :options="sites" placeholder="Site" showClear class="w-28 custom-dropdown small" overlayClass="custom-dropdown-panel small" @change="onSiteChange"/>
         <Select v-model="filterStore.selectedSdwt" :options="sdwts" placeholder="SDWT" :disabled="!filterStore.selectedSite" showClear class="w-32 custom-dropdown small" overlayClass="custom-dropdown-panel small" @change="onSdwtChange"/>
         <Select v-model="filters.eqpId" :options="eqpIds" filter placeholder="EQP ID" :disabled="!filterStore.selectedSdwt" showClear class="w-32 custom-dropdown small" overlayClass="custom-dropdown-panel small" @change="onEqpChange"/>
         <div class="w-px h-6 bg-slate-200 dark:bg-zinc-700 mx-1 shrink-0"></div>
         <Select v-model="filters.lotId" :options="lotIds" filter placeholder="Lot ID" :disabled="!filters.eqpId" showClear class="w-40 custom-dropdown small" overlayClass="custom-dropdown-panel small" @change="onLotChange"/>
    ...
    <div v-if="filters.lotId" class="flex flex-1 min-h-0 gap-3 pb-2 overflow-hidden lg:flex-row flex-col animate-fade-in">
      ...
      <div class="flex flex-col w-full lg:w-64 shrink-0 bg-white dark:bg-[#111111] border border-slate-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden h-[726px] lg:h-auto">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-4">
           <div class="space-y-3">
             <div class="flex items-center gap-2 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
               <span class="w-4 h-4 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-[9px]">1</span> Process Condition
             </div>
           ...
           <div class="space-y-2">
             <div class="flex items-center gap-2 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
               <span class="w-4 h-4 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-[9px]">2</span> Measurement Point
             </div>
             <div class="pl-2">
               <div v-if="isPointsLoading" class="flex items-center gap-2 px-2 py-1.5 text-xs text-slate-400"><i class="pi pi-spin pi-spinner"></i> Loading Points...</div>
           ...
           <div class="flex flex-col flex-1 min-h-0">
             <div class="flex items-center justify-between mb-2">
               <div class="flex items-center gap-2 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
                 <span class="w-4 h-4 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-[9px]">3</span> Wafers ({{ selectedWafers.length }})
               </div>
             ...
             <div class="pl-2">
               ...
               <div v-else class="overflow-y-auto custom-scrollbar flex-1 pr-1 space-y-1 max-h-[200px]">
                  <div v-for="w in waferList" :key="w" @click="toggleWafer(w)" 
                       class="flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer transition-all border group"
                       :class="selectedWafers.includes(w) ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800/50' : 'border-transparent hover:bg-slate-50 dark:hover:bg-zinc-800/50'">
      ...
      <div class="flex flex-col flex-1 gap-3 overflow-hidden h-[726px] lg:h-auto">
        ...
        <div class="flex-[3] min-h-0 bg-white border shadow-sm rounded-xl dark:bg-[#111111] border-slate-200 dark:border-zinc-800 relative flex flex-col">
          <div class="flex items-center justify-between px-4 py-2 border-b border-slate-100 dark:border-zinc-800 shrink-0 bg-slate-50/50 dark:bg-zinc-900/30">
            <div class="flex items-center gap-3">
              ...
              <div class="flex items-center gap-2 ml-4">
                <span class="text-[10px] font-bold text-slate-400">Golden Ref:</span>
                <InputSwitch v-model="showGoldenRef" class="scale-75" @change="toggleGoldenRef"/>
              </div>
            </div>
            <div class="flex items-center gap-2">
          ...
          <div class="relative flex-1 w-full min-h-0 bg-slate-50/30 dark:bg-black/20">
            <div v-if="!hasSearched" class="absolute inset-0 flex flex-col items-center justify-center text-slate-400 opacity-60 select-none">
              <div class="w-16 h-16 bg-slate-100 dark:bg-zinc-800/50 rounded-full flex items-center justify-center mb-3"><i class="pi pi-chart-line text-2xl text-slate-300 dark:text-zinc-600"></i></div>
            ...
            <transition name="fade">
               <button v-if="isZoomed" @click="resetZoom" class="absolute top-3 right-3 bg-indigo-500 hover:bg-indigo-600 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg shadow-md flex items-center gap-1.5 transition-all z-10 cursor-pointer"><i class="pi pi-refresh text-[9px]"></i> Reset Zoom</button>
            </transition>
          </div>
        </div>
...
const isReadyToSearch = computed(() => filters.lotId && filters.cassetteRcp && filters.stageGroup && filters.pointId && selectedWafers.value.length > 0);

// --- New State for Analysis ---
const tableData = ref<any[]>([]);
const tableColumns = ref<string[]>([]); // 동적 컬럼 목록 저장용
...
const searchData = async () => {
  ...
  try {
    ...
    // [수정] 차트 시리즈 name을 Slot #으로 변경
    chartSeries.value = data ? data.map((d: any) => ({
      ...d,
      name: `Slot #${d.waferId}`
    })) : [];
    ...
    if (data && data.length > 0) {
      ...
      const allKeys = Object.keys(firstMeta).filter(k => 
        !['timestamp', 'lotId', 'gof'].includes(k) // 고정 컬럼 제외
      );
  ...
  try {
    ...
    if (golden && golden.wavelengths && golden.values) {
      ...
      goldenSeries.value = {
        ...
        data: dataPoints,
        showSymbol: false,
        // [수정] Z-Index를 20으로 올려 차트 앞으로 (필요시 조정)
        // 사용자가 배경에 깔리는 것을 원한다면 0, 잘 보이길 원하면 높은 값
        // 질문에서 "아무 변화가 없다"고 했으므로 확실히 보이게 z: 20 + solid line으로 변경 제안
...
const onRowSelect = async (event: any) => {
  ...
  try {
    ...
    if (genData) {
      // [수정] 모델 데이터 이름도 Slot #으로 변경
      genData.name = `Model (Slot #${wid})`;
      genSeries.value = genData;
    }
...
const chartOption = computed(() => {
  ...
  let series = chartSeries.value.map((s) => {
  ...
  if (goldenSeries.value) series.push(goldenSeries.value); // [수정] 맨 뒤에 추가하여 가장 위 레이어에 오도록 (Z값 영향도 있음)
  if (genSeries.value) series.push(genSeries.value);
  ...
  return {
    ...
    tooltip: { 
      ...
      formatter: (params: any[]) => {
        ...
        params.forEach(p => {
          if (p.seriesName === 'Golden Ref' || p.seriesName.startsWith('Model')) {
             html += `<div style="color:${p.color}" class="text-[10px]">● ${p.seriesName}</div>`;
             return;
          }

          // [수정] Slot # 파싱 및 Intensity 값 표시
    ...
    dataZoom: [
      ...
      { type: "slider", xAxisIndex: 0, bottom: 0, height: 16, borderColor: "transparent", fillerColor: "rgba(99, 102, 241, 0.1)" },
    ],
    xAxis: {
      type: "value",
      name: "Wavelength (nm)",
